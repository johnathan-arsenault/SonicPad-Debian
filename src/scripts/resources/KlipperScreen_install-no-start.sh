...
PYTHON="python3-virtualenv virtualenv python3-setuptools"
...
OPTIONAL="xserver-xorg-legacy fonts-nanum fonts-ipafont libmpv-dev polkitd network-manager"
...
install_packages()
{
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y $XSERVER
    if [ $? -eq 0 ]; then
        echo_ok "Installed X"
    else
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y $OPTIONAL
    echo $_
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y $FBDEV
    if [ $? -eq 0 ]; then
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y $PYGOBJECT
    if [ $? -eq 0 ]; then
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y $MISC
    if [ $? -eq 0 ]; then
